...
`get_market_odds(top, n_sims, blend_weight)` compara o título com o MERCADO:
busca o evento `world-cup-winner` do Polymarket (probabilidades implícitas nos
preços, vig removido), exibe modelo x mercado x blend (pool logarítmico com
...
zebras e goleadas aparecem na frequência estatística esperada, sinalizados em
`flags` e contabilizados em `summary`. Jogos de grupo já disputados ficam
fixados; `seed` torna o sorteio reprodutível.
